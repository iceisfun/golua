...
assert(err:match(":%d+: C stack overflow"), "expected location prefix, got: " .. tostring(err))

-- Also test simple recursion
local function g() g() end
local ok2, err2 = pcall(g)
assert(not ok2)
assert(err2:match(":%d+: C stack overflow"), "expected location prefix, got: " .. tostring(err2))
