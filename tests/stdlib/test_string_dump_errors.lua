...
assert(not ok)
assert(err:find("function expected, got number"), "got: " .. tostring(err))

local ok2, err2 = pcall(string.dump, "hello")
assert(not ok2)
assert(err2:find("function expected, got string"), "got: " .. tostring(err2))

local ok3, err3 = pcall(string.dump, true)
assert(not ok3)
assert(err3:find("function expected, got boolean"), "got: " .. tostring(err3))

local ok4, err4 = pcall(string.dump, nil)
assert(not ok4)
assert(err4:find("function expected, got nil"), "got: " .. tostring(err4))

local ok5, err5 = pcall(string.dump, {})
assert(not ok5)
assert(err5:find("function expected, got table"), "got: " .. tostring(err5))

-- Error: native (C) function
local ok6, err6 = pcall(string.dump, print)
assert(not ok6)
assert(err6:find("unable to dump"), "got: " .. tostring(err6))

local ok7, err7 = pcall(string.dump, type)
assert(not ok7)
assert(err7:find("unable to dump"), "got: " .. tostring(err7))
