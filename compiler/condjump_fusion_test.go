...
func TestVMImmediateCompareOpcodes(t *testing.T) {
	// Reference semantics (lua-5.4.8/src/lvm.c, op_orderI / OP_EQI):
	//   LTI: R[A] <  sB      LEI: R[A] <= sB
	//   GTI: R[A] >  sB      GEI: R[A] >= sB
	//   EQI: R[A] == sB (numbers only; other types are never equal)
	// Every expectation below was cross-checked against /usr/bin/lua5.4.8
...
// /usr/bin/lua5.4.8 on the identical source.
...
`
	// Reference output from lua5.4.8.
...
`
	// Reference output from lua5.4.8 (t == 5 and t == "s" never call __eq:
...
// for LTI/LEI and as the first for GTI/GEI). Verified against lua5.4.8.
...
return table.concat(out, " ")
`
	// lua5.4.8: n is read AFTER bump() runs (both operands are on the stack
...
return table.concat(out, " ")
`
	// Reference output from lua5.4.8 on identical source.
...
func TestCountHookInstructionCounts(t *testing.T) {
	cases := []struct {
		...
		ref  int // lua5.4.8 count for the same source
		was  int // GoLua count before fusion
	}{
		// Totals a count-1 hook observes between sethook/sethook(). The `ref`
		// column is /usr/bin/lua5.4.8 on identical source; it is documentation,
	...
	for _, tc := range cases {
		...
		if got != strconv.Itoa(tc.want) {
			t.Errorf("%s: count hook saw %s instructions, want %d (lua5.4.8: %d, before fusion: %d)",
