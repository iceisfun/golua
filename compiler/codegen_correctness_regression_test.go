...
// OP_CLOSE, parentheses turning a live local read into a snapshot, a unary
// operator evaluating its operand into the destination local, and
// front-to-back `global a, b = ...` stores.
//
...
// register on top of the receiver copy at base+1.
...
// silently stored into unrelated registers instead of raising.
...
// must be decided when the block ends, not when the jump is compiled.
...
// instruction that uses it rather than snapshotted into a temporary.
...
// value while a metamethod runs (and error messages blame the operand).
...
// TestGlobalDeclStoresBackToFront covers `global a, b, c = ...`: reference Lua
// emits each name's check-and-store on the way out of its recursion, so the
// last name is checked first and an error leaves no earlier name committed.
func TestGlobalDeclStoresBackToFront(t *testing.T) {
	checkChunk(t, "no partial commit", `
a = 1
local f = load("a = 1 global b, c, a = 10, 20, 30")
local ok = pcall(f)
return tostring(ok) .. " " .. tostring(b) .. " " .. tostring(c)
`, "false nil nil")

	checkChunk(t, "blames the last name", `
a, b = 1, 2
local f = load("global a, b = 10, 20")
local ok, err = pcall(f)
return tostring(ok) .. " " .. tostring(err):match("global '%a+' already defined")
`, "false global 'b' already defined")

	checkChunk(t, "successful multi-store", `
local f = load("global gx, gy, gz = 1, 2, 3 return gx + gy + gz")
return tostring(f())
`, "6")
}
...
// referenceLua locates a PUC-Rio Lua 5.5 binary to differential-test against.
func referenceLua() string {
	for _, name := range []string{"lua5.5.0", "lua5.5"} {
		path, err := exec.LookPath(name)
		if err != nil {
			continue
		}
		out, err := exec.Command(path, "-v").CombinedOutput()
		if err == nil && strings.Contains(string(out), "Lua 5.5") {
			return path
		}
	}
	return ""
...
// interpreter. It is skipped when no Lua 5.5 binary is installed.
func TestCodegenRandomDifferential(t *testing.T) {
	ref := referenceLua()
	if ref == "" {
		t.Skip("no PUC-Rio Lua 5.5 binary found; skipping differential run")
	}
	dir := t.TempDir()
