...
// validHeader writes the exact binary-chunk header (through the top-level
...
func validHeader(buf *bytes.Buffer) {
	...
	buf.WriteByte(4)
	_ = binary.Write(buf, binary.LittleEndian, int32(-0x5678))
	buf.WriteByte(4)
...
func loadFunctionPrefix(buf *bytes.Buffer) {
	putVarint(buf, 0) // source: size==0 reuse marker
	putVarint(buf, 0) // reuse index 0 -> empty source
	putVarint(buf, 0) // LineDef
	putVarint(buf, 0) // LastLine
	buf.WriteByte(0)  // NumParams
	buf.WriteByte(0)  // vararg flags
