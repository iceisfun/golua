...
// OS device semantics (e.g. the official Lua conformance suite's flush tests).
// It is deliberately NOT a safe embedding default: do not use it to run
...
func (f *fullFile) Flush(ctx context.Context) error {
	...
	if f.writer != nil {
		...
		if err != nil {
			...
			// succeeds (see the /dev/full flush test in the Lua suite).
