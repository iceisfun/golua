...
func TestValueTypes(t *testing.T) {
	tests := []struct {
		...
		{False, "boolean"},
		{NewInt(42), "number"},
		{NewFloat(3.14), "number"},
		{NewString("hello"), "string"},
		{NewTable(NewEmptyTable()), "table"},
	}
	for _, tc := range tests {
		if tc.v.Type() != tc.typ {
			t.Errorf("expected %s, got %s", tc.typ, tc.v.Type())
		}
	}
}
...
func TestValueString(t *testing.T) {
	...
	for _, tc := range tests {
		if tc.v.String() != tc.expect {
			t.Errorf("expected %s, got %s", tc.expect, tc.v.String())
		}
	}
}
...
func TestValueEquality(t *testing.T) {
	tests := []struct {
		...
		{NewInt(42), NewFloat(42.0), true},
		{NewString("hello"), NewString("hello"), true},
		{NewString("hello"), NewString("world"), false},
		{Nil, NewInt(0), false},
	}
	for _, tc := range tests {
		if tc.v1.Equal(tc.v2) != tc.expect {
			t.Errorf("%v == %v: expected %v", tc.v1, tc.v2, tc.expect)
		}
	}
}
...
func TestTableOperations(t *testing.T) {
	...
	// Set and get by int
	tbl.SetInt(1, NewInt(10))
	if tbl.GetInt(1).AsInt() != 10 {
		t.Error("int key failed")
	}

	// Set and get by string
	tbl.SetString("name", NewString("test"))
	if tbl.GetString("name").AsString() != "test" {
		t.Error("string key failed")
	}

	// Get missing key returns nil
	if !tbl.Get(NewInt(999)).IsNil() {
