...
// earlySyntaxCheckInterval and earlySyntaxCheckInterval2 control when we try
// to compile accumulated reader source to detect definitive syntax errors in
// infinite readers. We check at two points; if the same error persists across
// both checks (meaning additional reader data didn't resolve a split token),
// we report it as definitive.
const earlySyntaxCheckInterval = 100
const earlySyntaxCheckInterval2 = 200
...
func luaLoad(v *vm.VM) int {
	...
	if chunk.IsString() {
		...
		for {
			...
			// Periodic syntax check: try to compile accumulated source to
			// catch definitive errors in infinite readers. We check at two
			// intervals — if the same non-eof error persists across both
			// checks (meaning additional reader data didn't resolve it),
			// report it. This avoids false positives from tokens split
			// across reader boundaries.
			if readerCalls == earlySyntaxCheckInterval || readerCalls == earlySyntaxCheckInterval2 {
				if len(builder) > 0 {
					...
					if syntaxErr != "" && !strings.HasSuffix(syntaxErr, "near <eof>") {
						if readerCalls == earlySyntaxCheckInterval {
							lastSyntaxErr = syntaxErr
						} else if syntaxErr == lastSyntaxErr {
							// Same error persisted — definitive
							v.Set(0, vm.Nil)
