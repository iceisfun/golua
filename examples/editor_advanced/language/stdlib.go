...
var (
	globals []StdlibEntry
	tables  map[string][]StdlibEntry
	allByFQ map[string]StdlibEntry // keyed by "parent.name" or just "name"
)
...
func init() {
	...
	allByFQ = make(map[string]StdlibEntry)
	for _, e := range globals {
		allByFQ[e.Name] = e
	}
	for parent, members := range tables {
		for _, e := range members {
			allByFQ[parent+"."+e.Name] = e
		}
	}
}
